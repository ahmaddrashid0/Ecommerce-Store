...
def place_order(request):
    if request.method == 'POST':
        ...
        payment_mode = "Cash on Delivery"  # Example payment mode
        ...
        # Generate a new incremental order ID
        last_order = Order.objects.filter(user=user).order_by('-order_id').first()
        if last_order:
            last_order_number = int(last_order.order_id[3:])  # Extract numeric part
            new_order_id = f"ORD{last_order_number + 1:04d}"
        else:
            new_order_id = "ORD0001"  # First order
        ...
        return redirect('home')  # This will now correctly reverse to the home page
    else:
        # Redirect to the cart page if the request method is not POST
...
def success(request):
    return render(request, 'success.html', {'message': "Your order has been placed successfully!"})

from django.shortcuts import render

def home(request):
    return render(request, 'home/index.html')  # Or any other template you want
...
def add_to_cart(request, uid):
    try:
        ...
        if size_variants.exists() and not variant:
        ...
        # If size variant exists, fetch it; otherwise, use None
        size_variant = None
